...
let isTouch = typeof window !== "undefined" && "ontouchstart" in window && !window.PointerEvent
||
typeof window !== "undefined" && "ontouchstart" in window && window.PointerEvent && window.innerWidth <= 768
...
export default {
  ...
  methods: {
    ...
    onStart(e) {
      ...
      let currentMouseDown = window.PointerEvent ? "mousedown" : "pointerdown"
  ...
  mounted() {
    ...
    if (this.isTouch || this.mouseDrag) {
      this.$refs["VueCarousel-wrapper"].addEventListener(
        this.isTouch ? "touchstart" : currentMouseDown,
        this.onStart
      );
    }
